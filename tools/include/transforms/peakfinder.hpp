...
class PeakFinder_template_bank {
private:
  float threshold; //Sigma threshold
  float min_freq;
  float max_freq;
...
public:
  PeakFinder_template_bank(float threshold, float min_freq, float max_freq, unsigned int size, int min_gap=30)
    :threshold(threshold), min_freq(min_freq), 
     max_freq(max_freq),min_gap(min_gap),max_cands(100000)
  {
    idxs.resize(max_cands);
  ...
  void find_candidates(HarmonicSums<float>& sums, SpectrumCandidates_templatebank& cands){
    for (int ii=0;ii<sums.size();ii++)
      find_candidates(*sums[ii],cands);
  }
